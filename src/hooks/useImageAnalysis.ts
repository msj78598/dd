...
export const useImageAnalysis = () => {
    ...
    const analyzeImage = async (imageInput: File | File[]) => {
        ...
        // 🧠 البرومبت الفني المطور (V6.1) - منع تسريب التعليمات والثرثرة
        ...
        🚨 قاعدة تحديد نوع العداد (مباشر أم محولات CT):
        للتفريق بين العداد المباشر وعداد محولات التيار (CT)، يُمنع الاعتماد على رؤية المحولات في الصورة فقط. يجب عليك قراءة "قيمة التيار المقنن" (Rated Current) المطبوعة بدقة على واجهة العداد:
        - إذا كانت القيمة منخفضة (مثل: 1.5(6)A أو 5(10)A)، فهذا يُصنف قطعاً كـ "عداد محولات تيار (CT)"، حتى لو لم تظهر المحولات في الصورة.
        - إذا كانت القيمة عالية (مثل: 10(100)A أو 20(100)A أو 15(100)A)، فهذا يُصنف كـ "عداد مباشر".
        ...
        🛑 تحذير صارم للإخراج النهائي (DO NOT ECHO PROMPT):
        يُمنع منعاً باتاً كتابة أي مقدمات، أو ترحيب، أو تقديم نفسك، أو تكرار مهامك. يجب أن يبدأ ردك فوراً ومباشرةً بكلمة "النتيجة النهائية:" ويلتزم بالهيكلية فقط.`;
    ...
    const askFollowUp = async (question: string) => {
        ...
        // 🛑 البرومبت الخاص بالمحادثة مع توسيع النطاق ومنع الثرثرة
        ...
        1. يجب أن يكون ردك محصوراً فـقـط في تفاصيل الصورة المرفقة ومكونات المنظومة.
        2. إذا كان سؤال الفني يخص موضوعاً خارجاً عن النطاق (سيارات، معلومات عامة، إلخ)، ارفض الإجابة ورد حرفياً بـ:
        "عذراً، أنا مخصص للرد على الاستفسارات الفنية المتعلقة بالمنظومة الكهربائية والحالة المرفقة فقط."
        3. يُمنع منعاً باتاً كتابة أي مقدمات مثل "بصفتي المستشار الفني...". أجب على السؤال التقني مباشرة وبشكل مختصر.`;
